...
function getArpScanResults() {
    ...
    const filterResult = lll.filter(item => !AIRPORT_CONFIG.notProxyIp.includes(item.ip));
    return filterResult
//<<
        return [];
    }
    ...
    const filterResult = results.filter(item => AIRPORT_CONFIG.notProxyIp.includes(item.ip));
    return filterResult;
}
